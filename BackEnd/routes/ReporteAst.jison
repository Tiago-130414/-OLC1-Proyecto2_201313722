...
CONTENIDOP 
    : SINTAXISCONTENIDO {if($1!=''){$$ = [$1];}else{$$ = [];}}
    | CONTENIDOP SINTAXISCONTENIDO {if($2!=''){$1.push($2)}; $$ = $1;}
    ;
...
LISTADO_IMPORT
    : LISTADO_IMPORT S_Punto DEFINE_IMPORT { $1.push($3); $$ =$1;}
    | DEFINE_IMPORT                         {$$ = [$1];}
;
...
/*----------------------------------------------------------------------CLASES----------------------------------------------------------------------*/
CLASES 
    : R_Class Identificador S_LlaveAbre  PN S_LlaveCierra {$$ = API.n_Clase($2,$4);} //{$$ = $1+ $2 + $3 + $4 + $5;}{console.log(JSON.stringify($1, null, 2));
;
...
LISTA_CLASES
    : LISTA_CLASES CONTENIDO_CLASE {if($2!=''){$1.push($2)}; $$ = $1;}
    | CONTENIDO_CLASE              {if($1==''){$$ = [];}else{$$ = [$1];}}
;
...
EXPRESION_G 
    ...
    | CONTENIDO_EXPRESION OP_Decremento %prec PRUEBA                                             { $$ = API.operacion_Unaria($1,TIPO_OPERACION.DECREMENTO); }
    | CONTENIDO_EXPRESION OP_Incremento %prec PRUEBA                                             { $$ = API.operacion_Unaria($1,TIPO_OPERACION.INCREMENTO); }
...
CONTENIDO_EXPRESION
    ...
    | Identificador S_ParentesisAbre OPCIONAL S_ParentesisCierra                                 {$$ = API.n_Funcion($1,$3);}
...
METODOS_LL
    ...
    | Identificador S_ParentesisAbre PARAMETROS_FUNC S_ParentesisCierra S_PuntoComa             {$$ = API.n_Funcion($1,$3);}
;
/*
REDUCCION
...
LLAMAR_METODOF_CLASE
    : Identificador S_ParentesisAbre PARAMETROS_FUNC S_ParentesisCierra S_PuntoComa {$$ = API.n_Funcion($1,API.n_Parametro($3));}// $1 + $2 + $3 + $4 + $5
;
...
LISTA_INS
    : LISTA_INS LISTA_INSTRUCCIONES {if($2!=''){$1.push($2)}; $$ = $1;}
    | LISTA_INSTRUCCIONES           {
        if(Array.isArray($1)){
            if($1!=''){
                $$ = $1;
...
TIPO_IMPRESION
    : R_Print                   {$$ = INSTRUCCIONES_CONST.IMPRIMIR;}
    | R_Println                 {$$ = INSTRUCCIONES_CONST.IMPRIMIR_LN;}//IMPRIMIR
;
